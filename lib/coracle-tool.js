...
import CoracleSVN from './coracle-svn';
import ChineseSetting from './localization/main'
...
export default {
  ...
  activate(state) {
    ...
    if (this.userName == null) {
      ...
      atom.notifications.addInfo('登录成功!');
    }
    ...
    /* 项目模板指令集. */
      ...
      /* 注销 */
      atom.commands.add('atom-workspace', 'coracle:exit',
              (event)=>(this.convertCommandToMethod({event:event})))
  ...
  /* 将指令解析为对应的参数与方法,指令与方法对应的规则为: 命令空间:方法名,参数1=值1,参数2=值2,
    ...
    synctoCloud({event}){
        ...
        git.cmd(projectRootPath,['svn','dcommit']).catch((err)  => {atom.notifications.addWarning('检测到云端代码与本地未提交代码有冲突',{'detail':'在解决每个冲突后,在左侧树状图右键该文件执行Git ->  Git add + commit,以使修改冲突真正生效'})
    ...
    chectoutfromGit({event}){
      // const NavigateTo = require('./dialogs/navigate-to-dialog');
      // const searchView = new NavigateTo();
      //
      ...
      // const finder = new CoracleFinder();
      // const finderView = atom.workspace.addModalPanel({
      //   item: finder.getElement(),
      //   visible: false
      // });
      // finderView.show();
    },
    ...
    chectoutfromCoracle({event}){
      ...
      const svn = new CoracleSVN(projectList);
      const svnView = atom.workspace.addModalPanel({
        item: svn.getElement(),
        visible: false
      });
      svn.emitter.on('didSelectItem',(project) => {
    ...
    exit(event){
      ...
      atom.notifications.addSuccess('注销用户成功!');
    }
