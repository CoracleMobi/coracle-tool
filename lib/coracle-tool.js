...
export default {
  ...
  opener (url) {
      if (Url.parse(url).protocol == 'coracle-create:') {
        const webview = new WebView({uri: 'http://127.0.0.1:8080/module_project'});
      ...
      if (Url.parse(url).protocol == 'coracle-welcome:') {
        const webview = new WebView({uri: 'http://127.0.0.1:8080/module_welcome'});
      ...
      if (Url.parse(url).protocol == 'coracle-sync:') {
        const webview = new WebView({uri: 'http://127.0.0.1:8080/module_sync'});
        webview.addIpcMessageListener('create-syncApp', (data) => {
            atom.notifications.addInfo('正在同步应用'+data.appId);
            this.welcome();
        });
  ...
  /* 将指令解析为对应的参数与方法,指令与方法对应的规则为: 命令空间:方法名,参数1=值1,参数2=值2,
    ...
    loginSuccess(){
      ...
      git.cmd(null, ['checkout','--username','coracle','--password','coracle@2017fk2','--depth','empty','svn://172.16.23.86/836f56995c2740df947262f82a3d3002_55'],'',false,false);
        this.welcome();
    },
