...
export default {
  ...
  opener (url) {
      ...
      if (urlInfo.protocol == 'coracle-cloud:') {
        ...
        this.corWebView.setUri(package.host+'/portal/console/'+urlInfo.query+'/compile');
        return this.corWebView;
      }

      if (urlInfo.protocol == 'coracle-cloud-build-apprunner:') {
        console.log(Url.parse(url));
        this.corWebView.setUri(package.host+'/portal/console/'+urlInfo.query+'/plugin?type=3');
        return this.corWebView;
      }

      if (urlInfo.protocol == 'coracle-module:') {

        this.corWebView.setUri(package.host+'/portal/console/'+urlInfo.query+'/plugin');
        return this.corWebView;
      }
  ...
  /* 将指令解析为对应的参数与方法,指令与方法对应的规则为: 命令空间:方法名,参数1=值1,参数2=值2,
    ...
    synctoCloud({event}){
        ...
        git.cmd(projectRootPath,['svn','dcommit','--username',userName]).catch((err)  => {atom.notifications.addWarning('检测到云端代码与本地未提交代码有冲突',{'detail':'在解决每个冲突后,在左侧树状图右键该文件执行Git ->  Git add + commit,以使修改冲突真正生效'})
    ...
    build(url,{event}){
      ...
      if ( ! fs.existsSync(Path.resolve(projectRootPath, "config.xml"))){
        ...
        let appIdInfo = configText.match(/widget.*appId\s*=\s*[\"\'](.*)[\"\']\s+pId\s*=\s*[\"\']([\d\w]+)[\"\']/)
        if (appIdInfo === null){
        ...
        this.destoryView();
        console.log(url);
        atom.workspace.open(url+'?'+pId+'/'+appId).then(view => {
         });
    },
