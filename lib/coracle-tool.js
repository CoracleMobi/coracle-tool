...
export default {
  ...
  opener (url) {
      ...
      if (urlInfo.protocol == 'coracle-cloud:') {
        ...
        const webview = new WebView({uri: 'http://172.16.23.86:8088/portal/project/'+urlInfo.query+'/compile'});
      ...
      if (urlInfo.protocol == 'coracle-module:') {
        const webview = new WebView({uri: 'http://172.16.23.86:8088/portal/project/'+urlInfo.query+'/plugin'});
  ...
  /* 将指令解析为对应的参数与方法,指令与方法对应的规则为: 命令空间:方法名,参数1=值1,参数2=值2,
    ...
    wifiInfo({event}){
      ...
      // atom.openDevTools()
          // .then(()=>{
            ...
            atom.notifications.addInfo(tip,{
              "detail":"提示: 请下载并打开AppRunner 或 自定义AppRunner, 点击小圆球, 输入 IP 和端口连接,以进行 WIFI 调试",
            })
          // })
    },
    ...
    loginSuccess(userName){
      //git.cmd(null, ['up','--username','coracle','--password','coracle2017fk'],'',false,false);
      git.cmd(null, ['co','svn://172.16.23.86/55e523a37c7947bb969d56cbdd0a9fe6_252',__dirname,'--username',userName,'--password','coracle2017fk'],'',false,false);
        this.welcome();
    },
