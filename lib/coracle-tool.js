...
export default {
  ...
  opener (url) {
      if (Url.parse(url).protocol == 'coracle-create:') {
        const webview = new WebView({uri: 'http://172.16.23.86:8088/portal/ide/project.html'});
            ...
            this.destoryView();
              ...
              ipcRenderer.on(responseChannel, function(event, path) {
                ...
                if (path != null) {
                  ...
                  fs.mkdir(dir);
                  git.cmd(null, ['svn','clone',appInfo.svn,'--no-minimize-url',dir])
                    ...
                    CoracleManager.init({appInfo:appInfo, output:workspacePath});
      ...
      if (Url.parse(url).protocol == 'coracle-welcome:') {
        const webview = new WebView({uri: 'http://172.16.23.86:8088/portal/ide/welcome.html'});
      ...
      if (Url.parse(url).protocol == 'coracle-cloud:') {
        const webview = new WebView({uri: 'http://172.16.23.86:8088/portal/project/55e523a37c7947bb969d56cbdd0a9fe6/252/compile'});
        return webview;
      }

      if (Url.parse(url).protocol == 'coracle-module:') {
        const webview = new WebView({uri: 'http://172.16.23.86:8088/portal/project/55e523a37c7947bb969d56cbdd0a9fe6/252/plugin'});

        return webview;
      }

      if (Url.parse(url).protocol == 'coracle-sync:') {
        const webview = new WebView({uri: 'http://172.16.23.86:8088/portal/ide/sync.html'});
  ...
  /* 将指令解析为对应的参数与方法,指令与方法对应的规则为: 命令空间:方法名,参数1=值1,参数2=值2,
    ...
    wifiInfo({event}){
      ...
      // atom.openDevTools()
          // .then(()=>{
            ...
            atom.notifications.addInfo(tip,{
              "detail":"还可在Xbuild控制台末尾随时查看;ip地址有可能有多个,哪个可用,取决你和电脑所处的网络",
            })
          // })
    },
    ...
    cloudBuild({event}){
        ...
        this.destoryView();
        that = this;
        atom.workspace.open('coracle-cloud://build').then(view => {
             this.webView = view;
         });
    },
    ...
    moduleManage({event}){
        ...
        this.destoryView();
        that = this;
        atom.workspace.open('coracle-module://manage').then(view => {
             this.webView = view;
        });
    },
