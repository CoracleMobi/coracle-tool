...
export default {
  ...
  activate(state) {
    ...
    this.svn = new CoracleSVN();
    this.svnView = atom.workspace.addModalPanel({
      item: this.svn.getElement(),
      visible: false
    });
  ...
  /* 将指令解析为对应的参数与方法,指令与方法对应的规则为: 命令空间:方法名,参数1=值1,参数2=值2,
    ...
    chectoutfromCoracle({event}){
      ...
      return (
        this.svnView.isVisible() ?
        this.svnView.hide() :
        this.svnView.show()
      );
      // const listView = require('./dialogs/searchList');
