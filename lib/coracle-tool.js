...
import WifiSync from './WifiSync';libCorPluginReminders.a
...
export default {
  ...
  this.corWebView.addIpcMessageListener('sync-projectInfo', ()=>{
    ...
    this.corWebView.getWebVieElement().send('sendProjectInfo',projectList);        
  });
  ...
  opener (url) {
      ...
      if (urlInfo.protocol == 'coracle-cloud:') {
        ...
        this.corWebView.setUri(package.host+'/portal/project/'+urlInfo.query+'/compile');
        return this.corWebView;
      }

      if (urlInfo.protocol == 'coracle-cloud-build-apprunner:') {
        console.log(Url.parse(url));
        this.corWebView.setUri(package.host+'/portal/project/'+urlInfo.query+'/plugin?type=3');
        return this.corWebView;
      }

      if (urlInfo.protocol == 'coracle-module:') {

        this.corWebView.setUri(package.host+'/portal/project/'+urlInfo.query+'/plugin');
        return this.corWebView;
      }
